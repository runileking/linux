...
static int ad7766_postdisable(struct iio_dev *indio_dev)
{
	...
	/*
	 * The PD pin is synchronous to the clock, so give it some time to notice
	 * the change before we disable the clock.
	 */
	msleep(10);
...
static int ad7766_set_trigger_state(struct iio_trigger *trig, bool enable)
{
	...
	if (ad7766->trigger_enabled == enable)
		return 0;

	ad7766->trigger_enabled = enable;
